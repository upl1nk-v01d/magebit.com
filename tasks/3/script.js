$(document).ready(function() {
    ...
    function send_email_data(email,chbox) {
		$.ajax({
			async: false, 
			type: "POST",
			data:{email:email,chbox:chbox},
			url: "mysql/db_insert_row.php",
    ...
    function chc_form_validation(){
        ...
        if ($('form').attr('class')!='not_valid'){ 
            ...
            change_headings_txt();
            send_email_data($(".input_email_address .input_txt").val(),$(".TOS .checkmark input").is(":checked"));
            window.open("data_page.php");
            $('form').remove();
        }
    }
